...
function mapUser(row: UserRow, hosted: Map<number, number>, joined: Map<number, number>): A.User {
  return { id: String(row.id), name: row.fullname || row.username, username: row.username, email: row.email, avatar: row.profile_image ?? "", status: userStatus(row), joinedAt: dateValue(row.create_at), lastActiveAt: dateValue(row.create_at), location: locationOf(row), eventsHosted: hosted.get(row.id) ?? 0, eventsJoined: joined.get(row.id) ?? 0 };
}
...
export async function getUsers(params?: { search?: string; status?: A.UserStatus | "all"; location?: string; signupDate?: "all" | "last30" | "last90" | "last180" | "thisYear"; participation?: "all" | "high" | "medium" | "low"; page?: number; pageSize?: number }): Promise<{ rows: A.User[]; total: number }> {
  ...
  const rows = userRows.map((row) => mapUser(row, counts.hosted, counts.joined)).filter((row) => {
    ...
    return (!search || [row.id, row.name, row.username, row.email].some((value) => value.toLowerCase().includes(search))) && (!params?.status || params.status === "all" || row.status === params.status) && (!params?.location || params.location === "all" || row.location === params.location) && new Date(row.joinedAt) >= limit && participationMatch;
  });
