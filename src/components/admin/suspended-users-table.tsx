...
import { getSuspendedUsers } from "@/lib/api";
      ...
      <div className="flex flex-wrap items-center gap-7 border-b border-border/80 pb-1">
        {topTabs.map((tab) => (
          <Link
            key={tab.href}
            href={tab.href}
            className={`border-b-2 pb-2 text-sm font-semibold transition-colors ${tab.href === "/users/suspended" ? "border-primary text-foreground" : "border-transparent text-muted-foreground hover:text-foreground"}`}
          >
            {tab.label}
          </Link>
        ))}
            ...
            {query.isLoading
              ...
              : paginated.map((user) => (
                  <TableRow key={user.id}>
                    ...
                    <TableCell className="text-sm">{user.suspendedReason ?? "—"}</TableCell>
                    <TableCell className="text-sm">{user.suspendedUntil ?? "—"}</TableCell>
                      ...
                      <div className="flex items-center justify-end gap-1">
                        ...
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            ...
                            <DropdownMenuItem className="text-amber-600">Extend Suspension</DropdownMenuItem>
                            <DropdownMenuItem className="text-rose-600" onSelect={() => setBanTarget(user)}>Ban Permanently</DropdownMenuItem>
                          </DropdownMenuContent>
                        </DropdownMenu>
                      </div>
                    </TableCell>
