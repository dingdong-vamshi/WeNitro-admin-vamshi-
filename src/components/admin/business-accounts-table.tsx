...
const allIndustries: BusinessIndustry[] = [
  "sports", "tech", "food", "fashion", "health", "education", "entertainment", "finance",
];

const topTabs: Array<{ label: string; value: BusinessStatus | "all" }> = [
...
export function BusinessAccountsTable({ initialStatus = "all" }: { initialStatus?: BusinessStatus | "all" }) {
        ...
        <Table>
          <TableHeader>
            <TableRow className="border-b border-border/50 bg-muted/50 hover:bg-muted/50">
              <TableHead className="pl-4 text-xs font-semibold uppercase tracking-wide text-muted-foreground">Business</TableHead>
          ...
          <TableBody>
            ...
            {/* Data rows */}
            {query.data?.rows.map((biz) => (
              <TableRow key={biz.id} className="border-b border-border/40 last:border-0">
                <TableCell className="pl-4 py-3">
                  <div className="flex items-center gap-3">
                    <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-gradient-to-br from-blue-400 to-indigo-500 text-xs font-bold text-white">
                      {biz.logo}
                    </div>
                    <div>
                      <p className="font-medium leading-tight text-sm">{biz.name}</p>
                      <p className="mt-0.5 text-xs text-muted-foreground">{biz.email}</p>
                    </div>
                  </div>
                </TableCell>
                <TableCell className="py-3 text-sm text-muted-foreground">{industryLabel[biz.industry]}</TableCell>
                <TableCell className="py-3 text-sm text-muted-foreground">{biz.city}</TableCell>
                <TableCell className="py-3">
                  <Badge variant={statusVariant[biz.status]}>{statusLabel[biz.status]}</Badge>
                </TableCell>
                <TableCell className="py-3 text-sm text-muted-foreground">
                  {new Date(biz.joinedAt).toLocaleDateString("en-IN", { day: "numeric", month: "short" })}
                </TableCell>
                <TableCell className="pr-4 py-3 text-right">
                  <Button variant="outline" size="sm" onClick={() => router.push(`/business/${biz.id}`)}>
                    <ExternalLink className="mr-1.5 h-3.5 w-3.5" />
                    {biz.status === "pending" ? "Review" : "View"}
                  </Button>
                </TableCell>
              </TableRow>
            ))}
          ...
          <div className="flex items-center gap-3 text-sm text-muted-foreground">
            <span>
              {query.data && query.data.total > 0
                ? `Showing ${showingStart}\u2013${showingEnd} of ${query.data.total} businesses`
                : "No businesses found"}
            </span>
