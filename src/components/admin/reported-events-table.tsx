...
export function ReportedEventsTable() {
  ...
  const query = useQuery({
    queryKey: ["reported-events", debouncedSearch, severity, page, pageSize],
    queryFn: () => getReportedEvents({ search: debouncedSearch, severity, page, pageSize }),
  });
        ...
        <Table>
          <TableHeader>
            <TableRow className="border-b border-border/50 bg-muted/50 hover:bg-muted/50">
              <TableHead className="pl-4 text-xs font-semibold uppercase tracking-wide text-muted-foreground">Event Name</TableHead>
              <TableHead className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Host</TableHead>
          ...
          <TableBody>
            ...
            {query.data?.rows.map((event) => (
              <TableRow key={event.id} className="border-b border-border/40 last:border-0">
                ...
                <TableCell className="py-3 text-sm">{event.host}</TableCell>
                <TableCell className="py-3">
                  <span className="font-semibold text-rose-600 dark:text-rose-400">{event.reportCount}</span>
                </TableCell>
                <TableCell className="py-3">
                  <Badge variant="outline" className="text-xs">{event.reason}</Badge>
                </TableCell>
                <TableCell className="py-3">
                  <ModerationSeverityBadge severity={event.severity} />
                </TableCell>
                <TableCell className="py-3 text-sm text-muted-foreground">{event.date}</TableCell>
                <TableCell className="pr-4 py-3 text-right">
                  <div className="flex items-center justify-end gap-1">
                    <Button variant="ghost" size="sm" onClick={() => router.push(`/events/${event.eventId}`)}>
                      View Event
                    </Button>
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end" className="w-44">
                        <DropdownMenuItem onSelect={() => router.push(`/events/${event.eventId}`)}>
                          <Eye className="mr-2 h-3.5 w-3.5" /> View Event
                        </DropdownMenuItem>
                        ...
                        <DropdownMenuItem className="text-amber-600 dark:text-amber-400">
                          <AlertTriangle className="mr-2 h-3.5 w-3.5" /> Warn Host
                        </DropdownMenuItem>
                        <DropdownMenuItem className="text-orange-600 dark:text-orange-400">
                          <XOctagon className="mr-2 h-3.5 w-3.5" /> Cancel Event
                        </DropdownMenuItem>
                        <DropdownMenuItem className="text-destructive">
                          <Trash2 className="mr-2 h-3.5 w-3.5" /> Delete Event
                        </DropdownMenuItem>
                        <DropdownMenuItem className="text-rose-700 dark:text-rose-300">
                          <UserX className="mr-2 h-3.5 w-3.5" /> Suspend Host
                        </DropdownMenuItem>
                        <DropdownMenuSeparator />
          ...
          <div className="flex items-center gap-3 text-sm text-muted-foreground">
            <span>
              {query.data && query.data.total > 0
                ? `Showing ${showingStart}–${showingEnd} of ${query.data.total} reported events`
                : "No reported events found"}
            </span>
